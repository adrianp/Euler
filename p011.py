...
def main():
  matrix, maxp = [[8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8],
...
[1, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52, 1, 89, 19, 67, 48]], 0
  ...
  for i in range(17):
    for j in range(17):
      #down; we don't need to check in the "up" direction also, as we would check the same numbers twice
      ...
      if prod > maxp:
        maxp = prod
      #diagonal2, notice that we need to check this also, even if there is no clue about it in the problem description
      if j > 2:
        prod = matrix[i][j]*matrix[i+1][j-1]*matrix[i+2][j-2]*matrix[i+3][j-3]
        if prod > maxp:
          maxp = prod
